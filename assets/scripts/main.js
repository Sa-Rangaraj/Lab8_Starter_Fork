...
function initializeServiceWorker() {
  ...
  //            "Using Service Workers" will help you here)
      try {
        const registerWorker = await navigator.serviceWorker.register('./sw.js');
...
async function getRecipes() {
  ...
  if (storedRecipes) {
      const foundRecipes = JSON.parse(storedRecipes);
      return foundRecipes;
  } 
  ...
  //            "await" again
          const retJson = await fetchedUrl.json();
  // A8. TODO - Add the new recipe to the recipes array
          fetchedRecipes.push(retJson);
